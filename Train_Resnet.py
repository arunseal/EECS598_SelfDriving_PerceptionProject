...
from Config import TRAIN_FILE_PATH,TEST_FILE_PATH,IMG_SIZE_ALEXNET,BATCH_SIZE,EPOCHS,LEARNING_RATE,OUTPUT_CLASSES,MODEL_SAVE_PATH
...
def Train():
    ...
    """
    # try:
    X_full,y_full,data_id_train= get_data_train_val(file_path=TRAIN_FILE_PATH,logger=logger)
    # steps = X.shape[0]   
    steps=len(X_full)
    ...
    testing = tf.placeholder(tf.bool,name='testing')
    # print(reuse)

    logger.info("Getting Predicted Probs")
    #Predicted Probs
    y_pred=Model_RESNET(x,y_true,hold_prob1,hold_prob2,logger,testing=testing)

    logger.info("Finding Loss")
    #Defining loss function
    regu_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_true,logits=y_pred),name='cross_entropy')
    cross_entropy = tf.add_n([cross_entropy_loss]+regu_losses)
    ...
    test_acc_list_total=[]
    test_acc_def_list,test_acc_fp_list=[],[]
    ...
    with tf.Session() as sess:
        ...
        for i in range(EPOCHS):
            ...
            for j in range(0,steps-remaining,BATCH_SIZE):
                sess.run([train_opt,cross_entropy],feed_dict={x:X[j:j+BATCH_SIZE] , y_true:y[j:j+BATCH_SIZE],hold_prob1:0.5,hold_prob2:0.5,testing:False})
            ...
            while(start<total):
                ...
                feed_dict ={x: test_batch_x,y_true:test_batch_y,hold_prob1:1.0,hold_prob2:1.0,testing:True})
            ...
            #Saving Model
            if test_acc_>=max(test_acc_list_total) and test_acc_>=0.95:
                try:
